...
var slp033CommonTypes = []string{
	"React", "Component", "FunctionComponent", "ReactNode", "ReactElement",
...
func (r SLP033) Check(d *diff.Diff) []Finding {
	...
	for _, f := range d.Files {
				...
				if ln.Kind == diff.LineAdd {
					...
					if strings.HasPrefix(strings.TrimSpace(strings.ToLower(content)), "import") {
						...
						if strings.Contains(content, "{") && strings.Contains(content, "}") {
							...
							// Handle default/named imports like: import React from 'react' or import React, { Component } from 'react'
							...
							if len(parts) >= 2 {
								...
								// Handle cases like "React, { Component }" or just "React"
		...
		// Check for usage of common types/hooks without imports
		for _, h := range f.Hunks {
			for _, ln := range h.Lines {
				if ln.Kind != diff.LineAdd {
					continue
				...
				for _, hook := range slp033ReactHooks {
					if strings.Contains(content, hook) && !importedItems[hook] {
				...
				for _, typ := range slp033CommonTypes {
					if strings.Contains(content, typ) && !importedItems[typ] {
